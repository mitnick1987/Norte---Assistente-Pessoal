...
describe('GET /setup/google/callback', () => {
  it('troca o code por tokens e devolve sucesso', async () => {
    ...
    const response = await app.inject({ method: 'GET', url: '/setup/google/callback?code=abc123&state=state-valido' });

    expect(response.statusCode).toBe(200);
    expect(service.completeSetup).toHaveBeenCalledWith('abc123', 'state-valido');
  ...
  it('rejeita chamada sem "code" antes de tentar completar o setup', async () => {
    ...
    const response = await app.inject({ method: 'GET', url: '/setup/google/callback?state=state-valido' });

    expect(response.statusCode).toBe(400);
    expect(service.completeSetup).not.toHaveBeenCalled();
  });

  it('aceita chamada sem "state" na query e deixa o serviço decidir a rejeição (validação de negócio, não de shape)', async () => {
    const { app, service } = buildApp({
      completeSetup: vi.fn().mockRejectedValue(new InvalidOAuthStateError()),
    });

    const response = await app.inject({ method: 'GET', url: '/setup/google/callback?code=abc123' });

    expect(service.completeSetup).toHaveBeenCalledWith('abc123', undefined);
    expect(response.statusCode).toBe(400);
    expect(response.json().error).toMatch(/state/i);
  });

  it('state divergente do gerado (CSRF/injeção de código) vira 400, nunca 200 mascarando o ataque', async () => {
    const { app, service } = buildApp({
      completeSetup: vi.fn().mockRejectedValue(new InvalidOAuthStateError()),
    });
    ...
    const response = await app.inject({
      method: 'GET',
      url: '/setup/google/callback?code=abc123&state=state-forjado',
    });

    expect(service.completeSetup).toHaveBeenCalledWith('abc123', 'state-forjado');
    expect(response.statusCode).toBe(400);
    expect(response.json().error).toMatch(/state/i);
  });
