...
- **Pendências/débitos:**
  ...
  - Segredo do webhook da Evolution aceito via query string além do header: a versão pinada 2.3.7 não garante entrega de headers customizados configurados via `webhook/set` (issue `EvolutionAPI/evolution-api#1933`, sem confirmação de fix). Decisão registrada em `webhook-provisioner.ts`; se uma versão futura confirmar suporte a `headers`, dá para migrar e manter só o header.
  - Não consegui editar `.env.example` diretamente nesta rodada (ACL do arquivo restringe escrita neste ambiente) — as chaves `HOST` e `BRAIN_WEBHOOK_URL` precisam ser adicionadas manualmente ao `.env.example` antes do merge (ver texto sugerido no diff desta entrega/PR).
- **Aprendizados:**
