...
**Status:** entregue · **Issue:** FEAT-001 · **Branch:** `feature/FEAT-001-fundacao` · **Data:** 2026-08-25
...
1. `docker compose -f infra/docker-compose.yml -f infra/docker-compose.local.yml --profile local up` — o override `docker-compose.local.yml` é o que publica a porta do painel da Evolution (nunca combinar em produção). O brain se autoprovisiona no webhook da Evolution assim que sobe; parear o QR code com o chip dedicado do Norte.
...
5. `GET /health`: confere que reflete DB ok, último tick do scheduler, estado da sessão WhatsApp e versão real do build (não `0.0.0`).

---

## Entrega

**Status:** entregue · **Data:** 2026-08-25

- **O que foi feito:** todo o escopo da spec, no backend. Scaffold (Node 22, TS strict, Fastify, better-sqlite3 em WAL, zod, pino, ESLint com `eslint-plugin-boundaries`, Vitest). `core/kernel`: `ModuleManifest` (com `events`) e `KernelRegistry` com composição determinística e `wireEvents` para o bus. `core/db`: WAL + runner de migrações com `down` testado. `core/scheduler`: poll de 30s, catch-up no boot, recorrência TZ-aware em `America/Sao_Paulo`. `core/outbox`: confirmação só pós-2xx, retry exponencial com backoff persistido em `retry_after`, delay anti-banimento de 10–45s + `sendPresence`, teto diário de proativas por dia civil `America/Sao_Paulo`, guard de reentrância + claim atômico por mensagem, alerta por e-mail ao esgotar retries. `core/channel` + adapter `whatsapp-evolution`: webhook validado com segredo em tempo constante, filtro de JID, dedup fail-closed quando falta `wa_message_id`, `sendText`/`sendPresence`/`getBase64FromMediaMessage`, watchdog de `CONNECTION_UPDATE`, autoprovisionamento do webhook no boot com retry/backoff. `modules/echo` (ping→pong). `GET /health` com versão do build resolvida do `package.json`. `infra/` com docker-compose (perfis `local`/`producao` + override `docker-compose.local.yml`), Dockerfile, Caddyfile, litestream.yml. CI sem as condições de bootstrap. README com setup local real.

  Suite verde: 30 arquivos de teste, 137 testes, cobertura ~98%. Review multi-agente encontrou 14 achados; os 9 bloqueantes/importantes/segurança foram corrigidos nesta mesma entrega (commit `06746b4`), os outros 3 viraram issues #2, #3 e #4.

- **Desvios da spec:**
  - **Segredo do webhook por query string, não só por header.** A spec (Escopo, linha do adapter) previa validação por segredo de webhook sem detalhar o transporte; a implementação aceita o segredo tanto em header (`x-webhook-secret`) quanto em query string (`?secret=...`), sempre comparado em tempo constante (`crypto.timingSafeEqual`). O motivo: a Evolution 2.3.7 (versão pinada pelo ADR-013) não garante a entrega de headers customizados configurados via `webhook/set` — issue upstream aberta e sem fix confirmado (`EvolutionAPI/evolution-api#1933`). Sem esse fallback, o autoprovisionamento do webhook no boot (`webhook-provisioner.ts`, novo — o brain chama `webhook/set` sozinho ao subir, com retry/backoff) não teria como garantir que o segredo chegasse em toda entrega real. Mitigação: o serializer de log (`core/logger.ts`) remove a query string da URL antes de logar, então o segredo não aparece em log mesmo vindo por esse caminho — coberto pelo teste S9 (`tests/security/webhook-security.test.ts`). Se uma versão futura da Evolution confirmar suporte a `headers` no `webhook/set`, dá para remover o fallback e manter só o header.
  - **Porta da Evolution só sai via override, nunca no compose base.** A spec já prescrevia os dois perfis do ADR-013; o detalhe que não estava explícito é que `infra/docker-compose.yml` (base) não publica nenhuma porta da Evolution — nem em `local` nem em `producao`. A porta só é publicada combinando `infra/docker-compose.local.yml` (`-f` extra), porque Compose não escopa `ports:` por `profiles:` dentro de uma única definição de serviço: condicionar a porta ao perfil exigia separar em arquivo próprio. Consequência prática: subir só o compose base nunca expõe o painel da Evolution, em nenhum perfil — reduz superfície por padrão (SECURITY.md §5).

- **PRs:** (preenchido pelo orquestrador ao abrir o PR)
- **Migrações:** `001_core_messages`, `002_core_settings`, `003_core_jobs`, `004_core_outbox_messages` — todas com `up`/`down` testados em `tests/unit/core-migrations.test.ts` e `tests/unit/migrator.test.ts`. Nenhuma migração nova na rodada de correções pós-review.
- **Pendências/débitos:**
  - `.env.example` não foi editado — a ACL do arquivo neste ambiente bloqueia escrita (e leitura por ferramenta). Faltam as chaves `HOST` e `BRAIN_WEBHOOK_URL`, que o boot do brain e o Compose já esperam. Adicionar manualmente antes do merge: `HOST=0.0.0.0` e `BRAIN_WEBHOOK_URL=http://brain:3000/webhook/evolution`.
  ...
  - `core/mcp` (servidor MCP) não existe ainda — corretamente fora de escopo (M2, ADR-014). O registry de tools já nasce transport-agnostic (`ToolDefinition` com schema zod), que é a restrição que esta fundação precisava cumprir.
  - Issues #2, #3 e #4: sugestões do review multi-agente sem risco de segurança ou correção, não bloqueantes — ver detalhe em cada issue.
- **Aprendizados:**
  - `eslint-plugin-boundaries` não resolve `import "./x.js"` para o `.ts` real sem `settings['import/resolver']` apontando para `eslint-import-resolver-typescript` — sem isso todo import interno vira tipo "unknown" e a regra `element-types` nunca dispara. O lint passa "verde" silenciosamente mesmo quebrado; vale testar manualmente uma violação proposital sempre que este plugin for configurado do zero. Achado durante a validação manual do lint como gate, corrigido antes de fechar a entrega (`eslint-import-resolver-typescript` adicionado como devDependency).
  ...
  - `docker-compose.yml` não escopa `ports:` por `profiles:` dentro de uma única definição de serviço — publicar uma porta só num perfil exige separar em override file (`-f` extra), não dá para condicionar no mesmo bloco de serviço.
  - Bind de rede dentro de container Docker é a armadilha clássica: `127.0.0.1` no processo é loopback do próprio container, não do host — isolamento de borda é responsabilidade do `ports:` do Compose, nunca do bind da aplicação.
