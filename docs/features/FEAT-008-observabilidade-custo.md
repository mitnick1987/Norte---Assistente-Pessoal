...
- **O que foi feito:** backend completo dos seis itens do escopo. (1) `EmailAlerter` envia e-mail de fato via `SmtpMailer` (nodemailer) ou `ResendMailer` (fetch nativo), com anti-flood por chave lógica em tabela nova; os três call-sites existentes (`alertDeliveryExhausted`/`alertRefreshFailure`/`alertAnchorRitualCapped`) passam a disparar envio real sem mudar assinatura. (2) Dead man's switch como job durável `dead_mans_switch` (recorrência a cada 5 min), gate de saúde reusando a mesma função do `/health`. (3) `ConnectionWatchdog` ganhou `onStateChange` e o estado `qr_requested` (evento `qrcode.updated` da Evolution, que antes nem era reconhecido pelo schema do webhook); mudança de estado para não-saudável dispara `alertSessionDown`. (4) `/health` agora usa `evaluateSystemHealth` (função única, `core/health/domain`) e responde 503 quando degradado — com grace period no boot (scheduler sem tick ainda, sessão `unknown`) para não reportar `degraded` nos primeiros instantes do processo. (5) Monitor de custo como job durável `cost_monitor` (recorrência horária): projeção mensal a partir de `messages`, preços em settings, alerta de orçamento e alarme de regressão de cache com prioridade mais alta. (6) Alerta de disco best-effort via `fs.promises.statfs`, sem dependência nova.
- **PRs:** (preencher pelo orquestrador ao abrir o PR)
- **Migrações:** `001_infra_ops_alert_dispatches` (tabela `infra_ops_alert_dispatches`, chave lógica + último disparo — anti-flood; `down` testado).
- **Pendências/débitos:**
  - Preços de Sonnet 5/Haiku 4.5 em settings usam como default o preço público vigente no fechamento desta entrega (Sonnet 5 no preço introdutório até 31/08/2026); ajustar via settings quando o preço introdutório expirar — não requer deploy, mas requer ação manual do dono (a spec já previa isso, não é bug).
  - Preço de `cache_read` por milhão de tokens não tem valor público tabelado por modelo na documentação consultada; o default usado (~10% do preço de input) é uma estimativa conservadora baseada na proporção usual de desconto de cache read da Anthropic — vale conferir contra a fatura real assim que houver uso em produção e ajustar via settings se divergir.
